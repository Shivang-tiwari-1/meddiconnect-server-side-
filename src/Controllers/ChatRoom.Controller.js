...
exports.sendMessage = asyncHandler(async (req, res) => {
  ...
  if (typeof message && typeof role === String) {
    ...
    return res
      .status(403)
      .json({ error: `error at ${__filename} at line ${getLineNumber()}` });
  }
  ...
  if (store_message) {
    ...
    return res
      .status(404)
      .json({
        error: `could not save messages error at ${__filename} at line ${getLineNumber()}`,
      });
  }
  ...
  if (io) {
    ...
    return res
      .status(400)
      .json({
        error: `could not retrive the io error at ${__filename} at line ${getLineNumber()}`,
      });
  }
